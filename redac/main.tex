...
Nous présenterons d'abord les contextes qui ont présidé à la réalisation de cet outil prévu pour être utilisé en classe à très brève échéance, ainsi que son architecture logique. Nous alternerons donc entre les côtés technique et fonctionnel. Le travail de développement informatique prenant une coloration didactique, nous donnerons une description logicielle de l'ensemble, ne fournissant des détails d'implémentation que sur les contributions essentielles que sont: \begin{enumerate}
    \item le générateur Javascript de codes Python aléatoires mais contrôlables et de niveau pédagogique, exécuté dans le navigateur, 
    \item le traducteur de code Python en graphique logigramme, écrit en Python exécuté par Pyodide et rendu dans le navigateur par Mermaid.js
    \item le serveur Flask (Python) enregistrant les interactions élèves face aux codes proposés pour construire une base de données didactiques MySQL. 
\end{enumerate}
\vspace{2cm}
...
Pour créer le rendu logigramme pour des codes valides, deux options étaient a priori envisageables:\begin{itemize}
    \item Soit à la génération du script, un logigramme statique (plus facile car le rendu logigramme est encapsulé avec la génération du code)
    \item Soit dynamique, modifiable en continu avec le code Python modifiable (ce qui alourdit l'UI qui devient moins lisible).
    \end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\textwidth, keepaspectratio]{architecture_generateur.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth, keepaspectratio]{gen_func_flow.png}
...
\end{itemize}
Enfin, voici ce que le navigateur rend, par Mermaid.js, à partir de la chaien de texte que mon traducteur de code lui a passé (pour rappel, le code Python MyCFG est exécuté côté navigateur grâce à Pyodide) :
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth, keepaspectratio]{logigramme_minimal.png}
    \caption{Rendu final dans le navigateur du logigramme correspondant au code \textit{minimal} ci-dessus.}
    \label{fig:logigramme_minimal}
\end{figure} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth, keepaspectratio]{foreach_forstr.png}
    \caption{Comparaison d'un ``for each'' théorique avec le rendu final Mermaid dans le navigateur pour une boucle ``for...str''}
    \label{fig:foreach_forstr}
\end{figure} 
...
\begin{longtable}{|p{3cm}|p{7cm}|p{6cm}|}
  \caption{Comparaison des logiques de construction de graphe de flux de contrôle}
