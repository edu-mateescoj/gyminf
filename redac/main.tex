...
% --- Réglages esthétiques du titre (titling) ---
...
\begin{titlepage}
  ...
  % AVEC LOGO ???
  \begin{center}
    % \includegraphics[height=1.6cm]{logo.pdf}\par\vspace{0.4cm}
    {\Large ?? UniFr , HEP ?}
  \end{center}
...
\vspace{2cm}
  % Pied de page uniquement sur la page de garde
  \pagestyle{fancy}
  \fancyhf{}
  \fancyfoot[L]{\small \textbf{Superviseur :} Patrick Wang, HEP Lausanne}
  \fancyfoot[R]{\small \textbf{Juré expert :} Micha Hersch, HEP Lausanne, Gymnase de Renens}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Point de sortie :} valeure de retour de \code{visit()} et \code{visit_body()}
    \begin{itemize}
        \item Chaque appel à \code{visit()} ou \code{visit_body()} retourne une liste d'identifiants de nœuds. C'est à partir de ces nœuds que le flux continuera. Une liste est nécessaire car une structure comme un if/else produit naturellement plusieurs points de sortie. 
    \end{itemize}
\end{itemize}
