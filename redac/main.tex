...
\begin{abstract}
Ce rapport présente un outil-auteur et sa réalisation faite dans le cadre du programme de formation GymInf. L'outil propose principalement quatre fonctionnalités :\begin{itemize}
    \item[\ding{51}] un générateur automatique (infinie) de code Python valide et de niveau pédagogique adapté aux besoins définis par l'utilisateur, exécuté dans le navigateur \textit{full front-end}.
    \item[\ding{51}] un traducteur de code Python en flowchart, exécuté et affiché dans le navigateur \textit{full front-end}.
    \item[\ding{51}] la constitution sur serveur d'une base de données enregistrant les codes Python et les interactions élèves correspondantes, laissant la possibilité de suivre individuellement l'évolution des élèves dans leurs interactions avec les codes proposés et les questions correspondantes.
...
\end{itemize}   
Ce rapport présentera les contextes (professionnel, personnel, pédagogique et pratique) qui ont présidé à la réalisation de cet outil, son architecture logique et donnera plus de détails sur les contributions essentielles que sont: \begin{enumerate}
    ...
    \item le traducteur de code Python en un rendu graphique \textit{flowchart} dans le navigateur.
\end{enumerate} 
\par
...
\subsubsection{Un générateur de codes Python, de questions pertinentes, de valeurs \& erreurs attendues}
\begin{enumerate}
...
L'application présentée dans ce travail de mémoire diffère des applications existantes par son panachage unique (à ma connaissance) de plusieurs aspects, proposant aux enseignants d'investir en un seul outil les dimensions Predict, Run, Investigate Modify de l'approche PRIMM d'introduction à la programmation :
\begin{itemize}
    ...
    \item la possibilité d'alterner entre les langages Python et flowchart, en ayant à l'écran les noms théoriques des éléments syntaxiques
    \item la possibilité de tracer le parcours des élèves, leurs réussites/échecs, leur ténacité
\end{itemize} .
...
\begin{description}
    ...
    Codex est une plateforme d'exercices et d'évaluation développée pour les écoles du secondaire II. Elle permet aux enseignants de créer des épreuves de programmation sécurisées, où les élèves soumettent leur code qui est ensuite évalué par des tests automatiques dans un environnement contrôlé.
    
    \item[Différenciation :] Notre application est volontairement \textbf{plus légère, plus ciblée sur l'expérimentation autonome par l'élève et axée sur la formation plutôt que l'évaluation}. Contrairement à Replit ou Codex, qui sont des solutions lourdes, basées sur une infrastructure client-serveur et requérant des comptes, notre outil peut fonctionner comme une application web statique, sans serveur d'exécution grâce à Pyodide, donc anonyme, et conçue pour une tâche unique : générer un exercice ponctuel. Il n'a pas vocation à gérer un cours ou à organiser des examens, mais à fournir une ressource formative que l'enseignant peut intégrer dans son propre cheminement.
\end{description}
...
En conclusion, la multitude des ressources pédagogiques disponibles est une source inépuisable d'inspiration

\clearpage
\section{Présentation générale de l'application}
Dans cette partie sera présentée d'abord l'interface utilisateur, puis l'architecture globale et enfin les choix technologiques
\begin{itemize}
    \item Présentation UI
    \item \sout{Présentation des fonctionnalités}
...
\begin{itemize}
    ...
    \item[\ding{108}] Enrichir les questions des "Défis": 
    \begin{enumerate}
    ...
    \item Améliorer l'interface pour augmenter l'engagement:
    \begin{enumerate}
        \item Rendre le \textit{flowchart} cliquable pour un meilleur rendu smartphone
        \item Améliorer la \textit{responsiveness}
    ...
    \begin{enumerate}
        ...
        \item les \texttt{try: except: else: finally:}, les \texttt{Raise}, les \texttt{Assert}, ...
